...
The design rationale lives in [`ontorag_flow_demo_plan.md`](https://github.com/nuri428/ontorag-demo/blob/main/ontorag_flow_demo_plan.md).
This page is the **runbook + annotated output log** — every script's
real output is captured below so you can read the demo end-to-end
without running it.

Three companion documents go deeper:

* **[Walkthrough](./walkthrough.md)** — *"how each answer is derived"* —
  14 questions × (data, query, result, interpretation).
* **[Implementation](./implementation.md)** — *"why the code looks the
  way it looks"* — 12 design decisions × (problem, options, choice,
  code applied, validation).
* **[Developer guide](./dev_guide.md)** — *"how to actually work on
  this"* — 5 change recipes + change-impact matrix + extension points +
  troubleshooting + dev cycle.
